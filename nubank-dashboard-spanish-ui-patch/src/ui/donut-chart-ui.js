...
function renderDonutLegendItem(item, index) {
  ...
  const movementLabel = Number(item.totalMovements) === 1 ? "movimiento" : "movimientos";
...
function renderDonutEmptyState(container) {
  container.innerHTML = `
    <div class="chart-empty-state">
      <strong>No hay participación de gastos disponible.</strong>
      <p>Carga movimientos de gasto o ajusta los filtros para mostrar porcentajes por categoría.</p>
    </div>
  `;
}
