...
export function renderTopExpenses(elements, topExpenses = []) {
  ...
  for (const expense of safeTopExpenses) {
    ...
    const amount = Math.abs(
      Number(expense.absoluteAmount ?? expense.amount ?? 0)
    );

    const description = expense.description ?? "Unknown movement";
    const date = expense.date ?? "Unknown date";
    const category = formatCategory(expense.category ?? "uncategorized");
    const source = expense.source ?? "Unknown source";

    item.innerHTML = `
      <div class="top-expense-main">
        <strong class="top-expense-description" title="${escapeHtml(description)}">
      ...
      <strong class="top-expense-amount">
...
function renderTopExpensesEmptyState(container) {
  container.innerHTML = `
    <div class="top-expense-empty-state">
      <strong>No top expenses available.</strong>
      <p>Load expense movements or adjust the filters to display the highest expenses.</p>
    </div>
  `;
}
