...
function renderMonthFilterOptions(elements, movements) {
  ...
  const months = Array.from(
    new Set(movements.map((movement) => movement.month).filter(Boolean))
  ).sort();
  ...
  elements.monthFilter.innerHTML = `
    ...
    ${months
      .map((month) => {
        ...
        return `
          <option value="${escapeHtml(month)}" ${selected}>
            ${escapeHtml(month)}
          </option>
        `;
      })
...
function renderSourceFilterOptions(elements, movements, files) {
  ...
  const sourcesFromMovements = movements
    .map((movement) => movement.source)
    .filter(Boolean);

  const sources = Array.from(
    new Set([...sourcesFromFiles, ...sourcesFromMovements])
  ).sort();
  ...
  elements.sourceFilter.innerHTML = `
    ...
    ${sources
      .map((source) => {
        ...
        return `
          <option value="${escapeHtml(source)}" ${selected}>
            ${escapeHtml(source)}
          </option>
        `;
      })
