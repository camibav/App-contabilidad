...
export function renderDashboardCharts(elements, stats = {}) {
  ...
  const categoryExpenses = byCategory
    ...
    .slice(0, 8)
    .map((group) => ({
      label: formatCategory(group.key),
      value: Number(group.expenses),
    }));

  renderBarChart(elements.expensesByMonthChart, monthlyExpenses, {
    emptyTitle: "No monthly expenses available.",
    emptyDescription:
  ...
  renderBarChart(elements.expensesByCategoryChart, categoryExpenses, {
    emptyTitle: "No category expenses available.",
    emptyDescription:
...
export function clearDashboardCharts(elements) {
  renderDashboardCharts(elements, {
    byMonth: [],
    byCategory: [],
  });
}
  ...
  for (const item of items) {
    ...
    row.innerHTML = `
      <div class="chart-bar-meta">
        <span class="chart-bar-label" title="${escapeHtml(item.label)}">
