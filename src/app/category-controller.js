...
export function createCategoryChangeHandler({ elements, state, renderDashboard }) {
  return function handleCategoryChange(event) {
    ...
    if (!targetMovement) {
      renderDashboard();
      setStatus(elements, "The selected movement could not be found.");
      return;
    }
...
function shouldApplyCategoryToSimilarMovements({
  ...
  const movementLabel = similarMovements.length === 1 ? "movement" : "movements";
  const formattedCategory = formatCategory(newCategory);
  const description = targetMovement.description ?? "Unknown movement";

  return window.confirm(
    `Apply "${formattedCategory}" to ${similarMovements.length} similar ${movementLabel}?\n\n` +
      `Base movement:\n${description}\n\n` +
      "If you cancel, only the selected movement will be updated."
  );
}
...
function shouldRememberCategoryRule({ targetMovement, newCategory }) {
  ...
  return window.confirm(
    `Remember "${formattedCategory}" for future movements with a similar description?\n\n` +
      `Pattern:\n${learnedRule.pattern}\n\n` +
      `Base movement:\n${description}\n\n` +
      "If you accept, future PDFs can use this learned rule automatically."
  );
}
...
function buildCategoryDebugMessage({ appliedToSimilar, learnedRuleWasSaved }) {
  if (appliedToSimilar && learnedRuleWasSaved) {
    return "--- CATEGORY UPDATED FOR SIMILAR MOVEMENTS AND LEARNED RULE SAVED ---";
  }

  if (appliedToSimilar) {
    return "--- CATEGORY UPDATED FOR SIMILAR MOVEMENTS ---";
  }

  if (learnedRuleWasSaved) {
    return "--- CATEGORY UPDATED AND LEARNED RULE SAVED ---";
  }

  return "--- CATEGORY UPDATED FROM SELECTOR ---";
}
...
function buildCategoryUpdateStatus({
  ...
  const formattedCategory = formatCategory(newCategory);
  const movementLabel = updatedCount === 1 ? "movement" : "movements";
  const baseMessage = appliedToSimilar
    ? `Category updated: ${formattedCategory}. Updated ${updatedCount} similar ${movementLabel}.`
    : `Category updated: ${formattedCategory}.`;

  if (!learnedRuleWasSaved) {
    return baseMessage;
  }

  return `${baseMessage} Learned rule saved for future PDFs.`;
}
