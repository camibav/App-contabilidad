...
export function getDashboardDataFromBackupPayload(payload) {
  ...
  if (!data || typeof data !== "object") {
    throw new Error("The backup file does not contain dashboard data.");
  }

  if (!Array.isArray(data.movements)) {
    throw new Error("The backup file does not contain a valid movements array.");
  }
...
function normalizeBackupData(data) {
  ...
  return {
    ...data,
    fileName: data?.fileName ?? files.at(-1)?.name ?? "Unknown source",
    files,
